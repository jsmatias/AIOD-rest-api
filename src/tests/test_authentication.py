"""Unittests for the behaviour of get_current_user_or_raise_exception()."""
...
from authentication import get_current_user_or_raise_exception, keycloak_openid, User
...
async def test_happy_path():
    with MockedKeycloak() as _:
        user = await get_current_user_or_raise_exception(token="Bearer mocked")
...
def test_get_current_user_leaks_no_information():
    ...
    """
    assert inspect.signature(get_current_user_or_raise_exception).return_annotation == User
...
async def test_inactive_user():
    with MockedKeycloak(type_=TestUserType.inactive) as _:
        with pytest.raises(HTTPException) as exception_info:
            await get_current_user_or_raise_exception(token="Bearer mocked")
...
async def test_unauthenticated():
    with pytest.raises(HTTPException) as exception_info:
        await get_current_user_or_raise_exception(token=None)
    assert exception_info.value.status_code == status.HTTP_401_UNAUTHORIZED
    assert (
